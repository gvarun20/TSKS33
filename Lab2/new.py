...
class NeighborhoodPredictor(BaseLinePredictor):
    ...
    def delta(self, i, j, common_users):
        ...
        if len(u) < max(10, self._u_min // 2):  # More flexible threshold for common users
            return 0
...
if __name__ == "__main__":
    ...
    # Adjusted hyperparameters for 1% test improvement
    u_min, L = 50, 100
    print(f"\n-- Movie Neighborhood Predictor (u_min = {u_min}, L = {L}) --")
    neighborhood_predictor = NeighborhoodPredictor(u_min, L)
    ...
    training_improvement = (rmse_baseline_training - rmse_neighborhood_training) / rmse_baseline_training * 100
    test_improvement = (rmse_baseline_test - rmse_neighborhood_test) / rmse_baseline_test * 100

    print(f"Training improvement: {training_improvement:.3f}%")
